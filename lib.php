...
class academicyear_course_observer {
  ...
  public static function course_created($eventdata) {
    ...
    $extractedcourseyear = self::extract_course_year($eventdata->idnumber);
    
    if (empty($extractedcourseyear)) {
      add_to_log(SITEID, "academicyear", "unable to extract course year", "", $eventdata->shortname.' ('.$eventdata->idnumber.')');
      return true;
    }
    
    // get the record for the course academic year
    if (!$academicyear = $DB->get_record('local_academicyear', array('startyear'=>$extractedcourseyear), 'id, title')) {
      add_to_log(SITEID, "academicyear", "no academic year found", "", $extractedcourseyear.' ('.$eventdata->idnumber.')');
      return true;
    }

    $courseacademicyear = new stdClass();
    $courseacademicyear->courseid = $eventdata->id;
    $courseacademicyear->academicyearid = $academicyear->id; 

    $courseacademicyear->id = $DB->insert_record('local_academicyear_course', $courseacademicyear, true);

    add_to_log(SITEID, "academicyear", "course assigned academic year", "", $eventdata->shortname.' ('.$academicyear->title.')');
  ...
  public static function course_deleted($eventdata) {
    ...
    // Log action.
    add_to_log(SITEID, "academicyear", "course deleted event handler", "", $eventdata->fullname);
